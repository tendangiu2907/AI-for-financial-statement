...
from config import API_URL, chat_endpoint
        ...
        if st.session_state.extraction_completed and st.session_state.table_data:
            ...
            with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                for table_name, records in st.session_state.table_data.items():
                    ...
                    safe_name = table_name[:31].replace('/', '_').replace('\\', '_').replace('?', '_').replace('*', '_')
...
with col2:
    ...
    with input_container:
        ...
        if user_input:
            ...
            with st.spinner("Thinking..."):
                ...
                result = send_message_to_chatbot(user_input, session_id)
    ...
    with chat_container:
        # Tạo container có chiều cao cố định cho tin nhắn
        #st.markdown('<div class="chat-messages-container" style="height: 500px; overflow-y: auto; display: flex; flex-direction: column-reverse;">', unsafe_allow_html=True)
        
        # Hiển thị tin nhắn theo thứ tự ngược lại để tin nhắn mới nhất xuất hiện ở dưới cùng
        for message in reversed(st.session_state.messages):
            if message["role"] == "user":
                st.markdown(
                    f'<div style="display: flex; justify-content: flex-end; margin-bottom: 10px;">'
                    f'<div style="background-color: #3498db; color: white; padding: 10px 15px; border-radius: 15px 0px 15px 15px; max-width: 80%;">'
                    f'{message["content"]}'
                    f'</div></div>', 
                    unsafe_allow_html=True
                )
                ...
                st.markdown(
                    ...
                    f'<div style="background-color: #f0f2f6; padding: 10px 15px; border-radius: 0px 15px 15px 15px; max-width: 80%;">'
                    f'{message["content"]}'
                    f'</div></div>', 
                    unsafe_allow_html=True
                )
